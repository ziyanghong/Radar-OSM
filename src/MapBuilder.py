...
#                        roadSplits[stroad].add(st[1])
    ...
    for (croad,splitSet) in roadSplits.iteritems():
        # debugPrintInfo = croad == 'ef7a8a66ce89e7c80f9ef6a7f7290aae9181bf87.0000+1'
        if debugPrintInfo:
        ...
        # if prevInt == '53ea6925499b775cff1e0728b6857a2cd164a89e':
        #     debugPrintInfo = True
        ...
        for (n,csplit) in enumerate(splits):
            ...
            newRoads[currRoad] = {'start_int':prevInt,'end_int':currInt, \
                                  ...
                                  'orig_key':croad, 'lanes':roads[croad]['lanes'], \
            ...
            if debugPrintInfo:
                ...
                print currRoad
                print '[prevInt] = {0}'.format(prevInt)
                print '[end_int] = {0}'.format(currInt)
                print
            prevInt = currInt
            prevSplit = csplit
        ...
        if np.abs(splits[-1] - croad_len) < 1e-10:
            saveSegments.add(currRoad)
        # if debugPrintInfo and currPos[1] >= 85.0074264827:
...
#                    print 'r = {0}, dist = {1}, denom = {2}, newTurnLen = {3}'.format(r,np.sqrt(distSq),denom,newturnLen)
    ...
    # print 'Finished processing in add_turn_segments'
    ...
    if alwaysSplitFull:
        ...
        # print 'Start to remove the old, unused intersections and connecting end roadbits'
        ...
        for cint in removeInts:
            ...
            for vt in int_roads:
                ...
                if vt in newRoads and vt not in saveSegments:
                    # a fix: RadarOSM
                    if 'ef7a8a66ce89e7c80f9ef6a7f7290aae9181bf87' in vt:
                        # print vt
                        # print allDel
                        # print cint
                        allDel = False
        ...
        # print 'Finished remove the old, unused intersections and connecting end roadbits'


#    remove_short_roads(newRoads,newIntersections,2.0/1000.0)
#    remove_short_roads(newRoads,newIntersections,1.0/1000.0)
    remove_short_roads(newRoads,newIntersections,0.5/1000.0) # original selection
#     remove_short_roads(newRoads,newIntersections,5.0/1000.0)
...
def remove_short_roads(roads,intersections,roadLenThresh = 1.0/1000.0):
    ...
    while numDel > 0:
        ...
        # print 'before remove_short_roads'
...
#                print 'adding {0}'.format(nint)
        ...
        for croad in delRoads:
            if croad == 'ef7a8a66ce89e7c80f9ef6a7f7290aae9181bf87.0001+1-s0':
                print 'delete road ={0}'.format(croad)
            del roads[croad] 
    if totalNumDel > 0:
...
def split_twoway_streets(roads,intersections):
    ...
    for (cnode,cnodeData) in roads.iteritems():
        if cnodeData['oneway'] >= 0:
            ...
            if cnodeData['oneway'] > 0:
                newRoads[cnode + '+1']['lanes'] = cnodeData['lanes']
            else:
                newRoads[cnode + '+1']['lanes'] = cnodeData['lanes'] / 2.0
        # if 'ebf7f98151cc81591ad7797bab720d3f8267be80' in cnode:
        #     print 'ebf7f98151cc81591ad7797bab720d3f8267be80+1 has {0} lanes and oneway = {1}'.format(cnodeData['lanes'], cnodeData['oneway'])
        if cnodeData['oneway'] <= 0:
            newRoads[cnode + '-1'] = deepcopy(cnodeData)
            newRoads[cnode + '-1']['oneway'] = 1
...
def build_osm_map(binFile,doSimplify = True,prunePolylineNodes = True, lat_range = None, lon_range = None):
    ...
    print 'projScale {0}'.format(projScale)   
    ...
    for (tN,t) in enumerate(tObjs):
        ...
        if t.isPolyline():
            tPoly = t.getPolyline()
            if 'lanes' in t.tag:
                lanes = float(t.tag['lanes'])
            else:
                lanes = 2
                # print t.tag['lanes'] # ziyang debug
            if 'oneway' in t.tag:
                # print 'oneway = {0}'.format(t.tag['oneway']) # ziyang debug
                owTag = t.tag['oneway']
            ...
            for (N,(prevInd,nextInd,segInfo,segErr)) in enumerate(segments):
                ...
                roads[roadKey] = {'start_int':prevSha,'end_int':nextSha,'oneway':cOneway, \
                                  ...
                                  'osm_type':t.type,'osm_kind':t.kind, 'lanes': lanes}
...
#    (roads,intersections) = add_turn_segments_old(roads,intersections)
    ...
    # for e in mg.edges_iter(data):
    #     print e

    # # debug ziyang
    # for (n, vt) in enumerate(mg.nodes_iter()):
    #     if 'lanes' in mg.node[vt]:
    #         print 'lanes = {0}'.format(mg.node[vt]['lanes'])
    #
    #     if 'oneway' in mg.node[vt]:
    #         print 'oneway = {0}'.format(mg.node[vt]['oneway'])
    #
