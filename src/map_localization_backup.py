...
def extractRelevantStreet(options, mapgraph, iniitial_gps, position_std):
    ...
    cwd = os.getcwd()
    directory = cwd + '/results/visualization/' + options.dataname + '/'
    fig_name = directory + 'map_visualization.png'
    fig_map.savefig(fig_name, dpi=600)
...
def do_plot_all_methods_for_one_sequence(options,cfgFile):
    ...
    sequence = cfgFile.replace('configs/radar/', '')
    methods = ['radar', 'odom']
    for method in methods:
        cfgFile =  'configs/' + method + '/' + sequence
        load_config_file(options,cfgFile)
        ...
        for curr_obs in baseData['obs']:
            ...
            for (p,curr_street,curr_pos,curr_ang) in topModes:
                ...
                if method == 'radar':
                    ax.scatter(curr_pos[0], curr_pos[1], s=0.2, c='green', label='Ours')
                elif method == 'odom':
                    ax.scatter(curr_pos[0], curr_pos[1], s=0.2, c='blue', label='Lost')
                else:
                    print 'something else'
        print 'Finished plotting method {0}\n'.format(method)
    for gtState in gtStates:
        gt = gtState['gps_position'].reshape(2)
        ax.scatter(gt[0], gt[1], s=0.2, c='red', label='GT')

    cwd = os.getcwd()
    directory = cwd + '/results/' + sequence
    fig_name = directory +  '/trajectories.png'
...
def do_compute_errors(options,args,topErrN):
    ...
    print np.mean(top_one_error[:,0]) * 1000
    # Todo ziyang added
